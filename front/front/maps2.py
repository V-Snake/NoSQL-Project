...
class AlternanceApp:
    def __init__(_self):
        _self.session = requests.Session()
        retries = Retry(total=5, backoff_factor=0.1)
        adapter = HTTPAdapter(max_retries=retries)
        _self.session.mount('http://', adapter)
        _self.session.mount('https://', adapter)
        _self.api_key = '13e7eb7e21f74cd6b4dc0f5ad0fcce30'  # Clé API OpenCage
        
    # Fonction pour obtenir les données depuis l'API avec mise en cache
    @st.cache_data
    def get_data(_self, api_url):
        try:
            response = _self.session.get(api_url, timeout=10)
    ...
    def haversine(_self, lon1, lat1, lon2, lat2):
    ...
    def filter_data_by_radius(_self, data, lat, lon, radius):
        filtered_data = []
        for item in data:
            try:
                ...
                distance = _self.haversine(lon, lat, item_lon, item_lat)
                if distance <= radius:
    ...
    def run(_self):
        ...
        formations_data = _self.get_data(formations_api_url)
        jobs_data = _self.get_data(jobs_api_url)
        effectif_data = _self.get_data(effectif_api_url)
        ...
        effectif_df['date'] = pd.to_datetime(effectif_df['date'].apply(_self.extract_date), errors='coerce')
        ...
        if section == "Cartes des Formations et Offres":
            ...
            if st.button('Obtenir les informations'):
                if address:
                    lat, lon = _self.get_coordinates(address)
                    if lat and lon:
                        st.success(f'Les coordonnées de l\'adresse sont :\nLatitude: {lat}\nLongitude: {lon}')
                        
                        filtered_formations_df = _self.filter_data_by_radius(formations_data, lat, lon, radius)
                        filtered_jobs_df = _self.filter_data_by_radius(jobs_data, lat, lon, radius)
        ...
        # Section 3: Orientation
            ...
            if st.button('Rechercher'):
                if metier:
                    ...
                    metiers_data = _self.get_data(metiers_api_url_full)
